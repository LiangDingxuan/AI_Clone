...
from shared.data_loader import get_chats_by_types, load_sorted_data
from shared.models import export_conversations
from approach_1_rule_based.sessionizer import sessionize_personal_chats
from approach_2_nlp_embeddings.sessionizer import sessionize_with_embeddings
...
def run_pipeline(
    ...
    if skip_step1 and sorted_path.exists():
        print(f"\n[Step 1/4] Skipping raw chat sorting (using existing {sorted_output}).")
    else:
        print("\n" + "-" * 80)
        ...
        export_sorted_chats(
            ...
            summary_path=str(PROJECT_ROOT / summary_output),
        )
    ...
    if skip_approach1:
        ...
        dm_output = PROJECT_ROOT / "approach_1_rule_based" / "output" / "sessions_personal_chat.json"
    ...
    if skip_approach2:
        ...
        sg_output = PROJECT_ROOT / "approach_2_nlp_embeddings" / "output" / "sessions_supergroup.json"
    ...
    if skip_approach3:
        ...
        grp_output = PROJECT_ROOT / "approach_3_ai_llm" / "output" / "sessions_group.json"
